...
output_dir = "Sorted_figures"
os.makedirs(output_dir, exist_ok=True)

# 1) Boxplot della distribuzione della colonna Number of Components (ordinato per mediana decrescente)
num_components_data = data['Number of Components'].dropna()
if len(num_components_data) > 1:
    plt.figure(figsize=(8, 6))
    plt.boxplot([num_components_data], tick_labels=["Dataset"])
    plt.title('Distribution of Number of Components')
    plt.ylabel('Number of Components')
    plt.savefig(f'{output_dir}/number_of_components_boxplot.jpeg')
    plt.close()

# 2) Boxplot della distribuzione della colonna Total Vulnerabilities (ordinato per mediana decrescente)
total_vulnerabilities_data = data['Total Vulnerabilities'].dropna()
...
# 3) Boxplot per ogni livello di severità (senza ordinamento specifico)
...
plt.savefig(f'{output_dir}/vulnerabilities_by_severity_boxplot.jpeg')
plt.close()
...
filtered_categories = [
    ...
    if len(data[data['Category'] == category]['Total Vulnerabilities'].dropna()) > 1
]

# Ordina le categorie in base alla mediana in ordine decrescente
category_medians = {
    category: data[data['Category'] == category]['Total Vulnerabilities'].median()
...
for i in range(num_plots):
    ...
    boxplot_data = [
        data[data['Category'] == category]['Total Vulnerabilities'].dropna()
